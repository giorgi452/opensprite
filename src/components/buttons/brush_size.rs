...
pub struct BrushSizeButton<T> {
    label: Box<dyn Widget<T>>,
}

impl<T: Data> BrushSizeButton<T> {
    pub fn new() -> Self {
        let label = Label::new("")
...
struct BrushSizeButtonController<T, F> {
    widget: Box<dyn Widget<T>>,
    on_click: Box<F>,
}

impl<T: Data, F: Fn(&mut EventCtx, &mut T, &Env)> Widget<T> for BrushSizeButtonController<T, F> {
